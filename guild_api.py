...
def _get_json(path: str, api_key: str, params: Optional[dict] = None) -> dict:
    ...
    try:
        ...
        raise GuildAPIError(f"Nexon API 호출 실패: {exc}") from exc
...
def fetch_guild_id(api_key: str, world_name: str, guild_name: str) -> str:
    ...
    if not guild_id:
        raise GuildAPIError("oguild_id를 찾지 못했습니다. world_name/guild_name을 확인하세요.")
    return guild_id
...
def fetch_guild_members(
    ...
    if not isinstance(members, list):
        raise GuildAPIError("길드원 목록 형식이 예상과 다릅니다.")
